...
def test_Cryptocoincandle_get_ticker_not_found(client):
    logger.info("GET .../ope/info - ticker NOT found")
    response = client.get("ope/candels?ticker=BTCTUSD&startDate=20001201000000&interval=1w")
...
def test_Cryptocoincandle_get_interval_not_found(client):
    ...
    response = client.get("ope/candels?ticker=BTC&startDate=20001201000000&interval=1d")
    logger.debug(f"GET Received response: {response.status_code} - {response.text}")
    assert response.status_code == HTTP_NOT_FOUND
...
def test_Cryptocoincandle_get_wrong_star_date(client):
    ...
    client.post('ope/intervals', json=intervals)
    logger.info("GET .../ope/candels - no data retrieved")
    response = client.get("ope/candels?ticker=BTC&startDate=200012010000&interval=1w")
    logger.debug(f"GET Received response: {response.status_code} - {response.text}")
...
def test_Cryptocoincandle_get_no_data(client):
    ...
    client.post('ope/intervals', json=intervals)
    logger.info("GET .../ope/candels - no data retrieved")
    response = client.get("ope/candels?ticker=BTC&startDate=20001201000000&interval=1w")
    logger.debug(f"GET Received response: {response.status_code} - {response.text}")
...
def test_Cryptocoincandle_get_OK(client):
    logging.debug("Add ticker BTC")
    coins = [{"name":"BTCTUSD", "ticker":"BTC"}]
    client.post("ope/cryptocoins", json=coins)
    logging.debug("Add interval Week/1w")
    intervals = [{'name_long': 'Week', 'name_short': '1w'}]
    client.post('ope/intervals', json=intervals)
    ...
    response = client.post("ope/candels", json=candels)
    logger.info("GET .../ope/candels - two record received")
    response = client.get("ope/candels?ticker=BTC&startDate=20250401000000&interval=1w")
    logger.debug(f"GET Received response: {response.status_code} - {response.text}")
...
def test_Cryptocoincandle_get_data_between(client):
    ...
    response = client.post("ope/candels", json=candels)
    logger.info("GET .../ope/candels - two record received")
    response = client.get("ope/candels?ticker=BTC&startDate=20250401000000&endDate=20250427235959&interval=1w")
    logger.debug(f"GET Received response: {response.status_code} - {response.text}")
    assert response.status_code == HTTP_OK
    data = response.get_json()
...
def test_Cryptocoincandle_get_between_no_data(client):
    ...
    response = client.post("ope/candels", json=candels)
    logger.info("GET .../ope/candels - two record received")
    response = client.get("ope/candels?ticker=BTC&startDate=20250401000000&endDate=20250427133800&interval=1w")
    logger.debug(f"GET Received response: {response.status_code} - {response.text}")
    assert response.status_code == HTTP_NO_CONTENT
