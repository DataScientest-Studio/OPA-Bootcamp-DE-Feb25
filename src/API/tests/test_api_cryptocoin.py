...
def test_Cryptocoin_get_no_content(client):
    logger.info("GET .../opa/cryptocoins")
    response = client.get('opa/cryptocoins')
...
def test_Cryptocoin_patch(client):
    ...
    response = client.patch('opa/cryptocoins/1', json=coins_update[0])
    assert response.status_code == HTTP_OK
    data = response.get_json()
...
def test_Cryptocoin_patch_integrity_err(client):
    ...
    response = client.patch('opa/cryptocoins/2', json=coins_update[0])
