...
class HistoricalData(Resource):
    ...
    def post(self):
        try:
            logger.info("POST: Create historical data request received")
...
#            next_id = (max_id or 0) + 1
            ...
            e_message = f"POST: Invalid date format {err}"
            logger.error(e_message)
            db.session.rollback()
            abort(HTTP_BAD_REQUEST, message=e_message)		
        except BadRequest as err:
            e_message = f"POST: Invalid JSON format {err}"
            logger.error(e_message)
            db.session.rollback()
            abort(HTTP_BAD_REQUEST, message="Invalid JSON format", errors=err.description)		
        except SQLAlchemyError as ex:
            e_message = f"POST: Database error occurred: [{str(ex)}], operation aborted"
            logger.error(e_message)
            db.session.rollback()
    ...
    def get(self):
        try:
            logger.info("GET: Retrieve all historical data request received")
            ...
            e_message = f"GET: Database error occurred: [{str(ex)}], operation aborted"
            logger.error(e_message)
            abort(HTTP_INTERNAL_SERVER_ERROR, message=e_message)
