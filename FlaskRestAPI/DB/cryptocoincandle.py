...
from FlaskRestAPI.DB.tables import CRYPTO_COIN_CANDLE_T, INTERVAL_T
...
class CryptoCoinCandleModel(db.Model):
    ...
    crypto_id = db.Column(db.Integer, db.ForeignKey(CRYPTO_COIN_CANDLE_T + '.id'), nullable=False)
...
def abort_if_invalid_date_format(arg_name: str, timestamp_str: str) -> datetime:
    ...
    if not re.match(date_pattern, timestamp_str):
        abort(HTTP_BAD_REQUEST, message=f"Argumnet [{arg_name}] error: invalid date format. Valid format :  YYYYMMDDhhmmss.")
    try:
       return datetime.strptime(timestamp_str, format)
    except ValueError:
        abort(HTTP_BAD_REQUEST, message=f"Argumnet [startDate] error: invalid date format. Valid format : YYYYMMDDhhmmss.")
...
def abort_if_invalid_args(args):
    ...
    if not ticker_db:
        abort(HTTP_NOT_FOUND, message=f"Argumnet [ticker] error: [{ticker_arg_val}] does not exist.")
    ...
    if not interval_db:
        abort(HTTP_NOT_FOUND, message=f"Argumnet [interval] error: [{interval_arg_val}] is not supported.")
